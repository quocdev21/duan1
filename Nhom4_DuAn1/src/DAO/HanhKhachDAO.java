...
public class HanhKhachDAO extends getConnection{
    ...
    public void loadTable(JTable tbl){
        try {
            String[] header = new String[]{"STT", "CCCD", "Họ tên", "Giới tính", "Điện thoại", "Email", "Địa chỉ"};
            String sql = "select ROW_NUMBER() Over (Order by CCCD), * from HANHKHACH";
    ...
    public HanhKhach timHKToDen(String ma) {
        try {
            String sql = "select * from HANHKHACH where CCCD = ?";
    ...
    public void tim(JTable tbl, String ma) {
        try {
            String sql = "select ROW_NUMBER() Over (Order by CCCD), * from HANHKHACH where CCCD like ?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, "%" + ma + "%");
            String[] header = new String[]{"STT", "CCCD", "Họ tên", "Giới tính", "Điện thoại", "Email", "Địa chỉ"};
