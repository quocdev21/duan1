...
public class frmDichVu extends javax.swing.JFrame {
    ...
    private void cbx() {
        ...
        try {
            int cao = 3;
            ArrayList<VeMayBay> arr_Ve = new DichVuRepo().layDS();
    ...
    private void bang() {
        new DichVuRepo().loadTable(tblDichVu);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new DichVuRepo().them(new DichVu(ma, mave, ten, giaf));
            if (kt == 1) {
                bang();
                DichVu dv = new DichVuRepo().timDVToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaDichVu.getText().isEmpty() && !txtMaVe.getText().isEmpty()) {
            ...
            String gia = txtGiaDV.getText().trim();
            float giaf = Float.parseFloat(gia);
            int kt = new DichVuRepo().sua(new DichVu(ma, mave, ten, giaf));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaDichVu.getText().trim();
                int kt = new DichVuRepo().xoa(new DichVu(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new DichVuRepo().hienThi(tblDichVu, sb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new DichVuRepo().tim(tblDichVu, txtTimKiem.getText().trim());
        editColumnWidth();
