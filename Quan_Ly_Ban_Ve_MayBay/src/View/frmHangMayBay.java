...
import Repository.HangMayBayRepo;
...
public class frmHangMayBay extends javax.swing.JFrame {
    ...
    private void bang() {
        new HangMayBayRepo().loadTable(tblHangMayBay);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new HangMayBayRepo().them(new HangMayBay(ma, ten, quocgia));
            if (kt == 1) {
                bang();
                HangMayBay hmb = new HangMayBayRepo().timHMBToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaHang.getText().isEmpty()) {
            ...
            String ten = txtTenHang.getText().trim();
            String quocgia = txtQuocGia.getText().trim();
            int kt = new HangMayBayRepo().sua(new HangMayBay(ma, ten, quocgia));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaHang.getText().trim();
                int kt = new HangMayBayRepo().xoa(new HangMayBay(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new HangMayBayRepo().hienThi(tblHangMayBay, hmb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new HangMayBayRepo().tim(tblHangMayBay, txtTimKiem.getText().trim());
        editColumnWidth();
