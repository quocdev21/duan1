...
public class frmHangVe extends javax.swing.JFrame {
    ...
    private void bang() {
        new HangVeRepo().loadTable(tblHangVe);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new HangVeRepo().them(new HangVe(ma, ten));
            if (kt == 1) {
                bang();
                HangVe hv = new HangVeRepo().timHVToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaHangVe.getText().isEmpty()) {
            String ma = txtMaHangVe.getText().trim();
            String ten = txtTenHangVe.getText().trim();
            int kt = new HangVeRepo().sua(new HangVe(ma, ten));
            if (kt == 1) {
                bang();
                editColumnWidth();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaHangVe.getText().trim();
                int kt = new HangVeRepo().xoa(new HangVe(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new HangVeRepo().hienThi(tblHangVe, hv, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new HangVeRepo().tim(tblHangVe, txtTimKiem.getText().trim());
        editColumnWidth();
