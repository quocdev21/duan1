...
import Repository.HanhKhachRepo;
...
public class frmHanhKhach extends javax.swing.JFrame {
    ...
    private void bang() {
        new HanhKhachRepo().loadTable(tblHanhKhach);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new HanhKhachRepo().them(new HanhKhach(cmnd, hoten, gt, sdt, email, diachi));
            if (kt == 1) {
                bang();
                HanhKhach hk = new HanhKhachRepo().timHKToDen(cmnd);
    ...
    private void capNhat() {
        if (!txtCMND.getText().isEmpty()) {
            ...
            String email = txtEmail.getText().trim();
            String diachi = txtDiaChi.getText().trim();
            int kt = new HanhKhachRepo().sua(new HanhKhach(cmnd, hoten, gt, sdt, email, diachi));
            if (kt == 1) {
                bang();
                editColumnWidth();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                ...
                int kt = new HanhKhachRepo().xoa(new HanhKhach(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new HanhKhachRepo().hienThi(tblHanhKhach, hk, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new HanhKhachRepo().tim(tblHanhKhach, txtTimKiem.getText().trim());
        editColumnWidth();
