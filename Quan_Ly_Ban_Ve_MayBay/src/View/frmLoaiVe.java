...
public class frmLoaiVe extends javax.swing.JFrame {
    ...
    private void bang() {
        new LoaiVeRepo().loadTable(tblLoaiVe);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new LoaiVeRepo().them(new LoaiVe(ma, ten));
            if (kt == 1) {
                bang();
                LoaiVe lv = new LoaiVeRepo().timLVToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaLoaiVe.getText().isEmpty()) {
            String ma = txtMaLoaiVe.getText().trim();
            String ten = txtTenLoaiVe.getText().trim();
            int kt = new LoaiVeRepo().sua(new LoaiVe(ma, ten));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaLoaiVe.getText().trim();
                int kt = new LoaiVeRepo().xoa(new LoaiVe(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new LoaiVeRepo().hienThi(tblLoaiVe, lv, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new LoaiVeRepo().tim(tblLoaiVe, txtTimKiem.getText().trim());
        editColumnWidth();
