...
public class frmMayBay extends javax.swing.JFrame {
    ...
    private void cbx() {
        ...
        try {
            int cao = 3;
            ArrayList<HangMayBay> arr_HMB = new MayBayRepo().layDS();
    ...
    private void bang() {
        new MayBayRepo().loadTable(tblMayBay);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new MayBayRepo().them(new MayBay(ma, mahang));
            if (kt == 1) {
                bang();
                MayBay lv = new MayBayRepo().timMBToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaMayBay.getText().isEmpty()) {
            ...
            String[] str = txtMaHangMB.getText().trim().split("-");
            String mahang = str[0].trim();
            int kt = new MayBayRepo().sua(new MayBay(ma, mahang));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaMayBay.getText().trim();
                int kt = new MayBayRepo().xoa(new MayBay(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new MayBayRepo().hienThi(tblMayBay, mb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new MayBayRepo().tim(tblMayBay, txtTimKiem.getText().trim());
        editColumnWidth();
