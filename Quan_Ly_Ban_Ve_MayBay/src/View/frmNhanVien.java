...
public class frmNhanVien extends javax.swing.JFrame {
    ...
    private void bang() {
        new NhanVienRepo().loadTable(tblNhanVien);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new NhanVienRepo().them(new NhanVien(manv, matkhau, vaitro, hoten, gioitinh, sdt, email, diachi, hinh));
            if (kt == 1) {
                bang();
                NhanVien hk = new NhanVienRepo().timNVToDen(manv);
    ...
    private void capNhat() {
        if (check()) {
            ...
            int kt = new NhanVienRepo().sua(new NhanVien(manv, matkhau, vaitro, hoten, gioitinh, sdt, email, diachi, hinh));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                ...
                int kt = new NhanVienRepo().xoa(new NhanVien(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new NhanVienRepo().hienThi(tblNhanVien, hk, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new NhanVienRepo().tim(tblNhanVien, txtTimKiem.getText().trim());
        editColumnWidth();
