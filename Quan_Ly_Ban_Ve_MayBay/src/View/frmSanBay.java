...
public class frmSanBay extends javax.swing.JFrame {
    ...
    private void bang() {
        new SanBayRepo().loadTable(tblSanBay);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new SanBayRepo().them(new SanBay(ma, ten, quocgia, diadiem));
            if (kt == 1) {
                bang();
                SanBay sb = new SanBayRepo().timSBToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaSanBay.getText().isEmpty()) {
            ...
            String quocgia = txtQuocGia.getText().trim();
            String diadiem = txtDiaDiem.getText().trim();
            int kt = new SanBayRepo().sua(new SanBay(ma, ten, quocgia, diadiem));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaSanBay.getText().trim();
                int kt = new SanBayRepo().xoa(new SanBay(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new SanBayRepo().hienThi(tblSanBay, sb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new SanBayRepo().tim(tblSanBay, txtTimKiem.getText().trim());
        editColumnWidth();
