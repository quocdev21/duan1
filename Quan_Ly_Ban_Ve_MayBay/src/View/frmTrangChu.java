...
public class frmTrangChu extends javax.swing.JFrame {
    ...
    private void cbx_CMND() {
        ...
        try {
            int cao = 3;
            ArrayList<HanhKhach> arr_HK = new HoaDonRepo().layDS_HK();
    ...
    private void cbx_MaNV() {
        ...
        try {
            int cao = 3;
            ArrayList<NhanVien> arr_NV = new HoaDonRepo().layDS_NV();
    ...
    private void bang() {
        new HoaDonRepo().loadTable(tblHoaDon);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new HoaDonRepo().them(new HoaDon(mahd, ngaylap, cmnd, manv));
            if (kt == 1) {
                bang();
                HoaDon hk = new HoaDonRepo().timHDToDen(mahd);
    ...
    private void capNhat() {
        if (!txtMaNV.getText().isEmpty()) {
            ...
            String[] str1 = txtMaNV.getText().trim().split("-");
            String manv = str1[0].trim();
            int kt = new HoaDonRepo().sua(new HoaDon(mahd, ngaylap, cmnd, manv));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String mahd = txtMaHD.getText().trim();
                int kt = new HoaDonRepo().xoa(new HoaDon(mahd));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new HoaDonRepo().hienThi(tblHoaDon, hd, row);
    ...
    private void initComponents() {
        ...
        tblHoaDon.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã hóa đơn", "Ngày lập", "Căn cước công dân", "Mã nhân viên"
            }
        ) {
        ...
        jLabel19.setText("Căn cước công dân");
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new HoaDonRepo().tim(tblHoaDon, txtTimKiem.getText().trim());
        editColumnWidth();
