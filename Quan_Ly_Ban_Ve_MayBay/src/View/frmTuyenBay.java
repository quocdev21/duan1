...
import Repository.TuyenBayRepo;
...
public class frmTuyenBay extends javax.swing.JFrame {
    ...
    private void cbx_MaSBDi() {
        ...
        try {
            int cao = 3;
            ArrayList<SanBay> arr_SB = new TuyenBayRepo().layDS();
    ...
    private void cbx_MaSBDen() {
        ...
        try {
            int cao = 3;
            ArrayList<SanBay> arr_HMB = new TuyenBayRepo().layDS();
    ...
    private void bang() {
        new TuyenBayRepo().loadTable(tblTuyenBay);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new TuyenBayRepo().them(new TuyenBay(ma, sbdi, sbden));
            if (kt == 1) {
                bang();
                TuyenBay hmb = new TuyenBayRepo().timTBToDen(ma);
    ...
    private void capNhat() {
        if (!txtMaTuyenBay.getText().isEmpty()) {
            ...
            String sbdi = str_sbdi[0].trim();
            String sbden = str_sbden[0].trim();
            int kt = new TuyenBayRepo().sua(new TuyenBay(ma, sbdi, sbden));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String ma = txtMaTuyenBay.getText().trim();
                int kt = new TuyenBayRepo().xoa(new TuyenBay(ma));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new TuyenBayRepo().hienThi(tblTuyenBay, tb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new TuyenBayRepo().tim(tblTuyenBay, txtTimKiem.getText().trim());
        editColumnWidth();
