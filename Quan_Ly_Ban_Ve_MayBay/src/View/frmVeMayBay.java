...
import Repository.VeMayBayRepo;
...
public class frmVeMayBay extends javax.swing.JFrame {
    ...
    private void cbx_HV() {
        ...
        try {
            int cao = 3;
            ArrayList<HangVe> arr_HV = new VeMayBayRepo().layDS_HV();
    ...
    private void cbx_LV() {
        ...
        try {
            int cao = 3;
            ArrayList<LoaiVe> arr_HV = new VeMayBayRepo().layDS_LV();
            for (int i = 0; i < arr_HV.size(); i++) {
    ...
    private void cbx_CB() {
        ...
        try {
            int cao = 3;
            ArrayList<ChuyenBay> arr_CB = new VeMayBayRepo().layDS_CB();
    ...
    private void bang() {
        new VeMayBayRepo().loadTable(tblVeMB);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new VeMayBayRepo().them(new VeMayBay(mave, mahang, maloai, machuyenbay, giaf));
            if (kt == 1) {
                bang();
                VeMayBay vmb = new VeMayBayRepo().timDVToDen(mave);
    ...
    private void capNhat() {
        if (!txtMaVeMayBay.getText().isEmpty()) {
            ...
            String gia = txtGiaBan.getText().trim();
            float giaf = Float.parseFloat(gia);
            int kt = new VeMayBayRepo().sua(new VeMayBay(mave, mahang, maloai, machuyenbay, giaf));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String mave = txtMaVeMayBay.getText().trim();
                int kt = new VeMayBayRepo().xoa(new VeMayBay(mave));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new VeMayBayRepo().hienThi(tblVeMB, vmb, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new VeMayBayRepo().tim(tblVeMB, txtTimKiem.getText().trim());
        editColumnWidth();
