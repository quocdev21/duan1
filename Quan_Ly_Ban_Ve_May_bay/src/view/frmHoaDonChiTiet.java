...
public class frmHoaDonChiTiet extends javax.swing.JFrame {
    ...
    private void cbx_MaHD() {
        ...
        try {
            int cao = 3;
            ArrayList<HoaDon> arr_HD = new HoaDonChiTietRepo().layDS_HD();
    ...
    private void cbx_MaVe() {
        ...
        try {
            int cao = 3;
            ArrayList<VeMayBay> arr_Ve = new HoaDonChiTietRepo().layDS_Ve();
            for (int i = 0; i < arr_Ve.size(); i++) {
                ...
                lbl.addMouseListener(new MouseListener() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        new HoaDonChiTietRepo().donGia(txtMaVeMB, txtDonGia);
                        new HoaDonChiTietRepo().thanhTien(txtSoGheDat, txtDonGia, txtThanhTien);
                    }
    ...
    private void bang() {
        new HoaDonChiTietRepo().loadTable(tblHoaDonChiTiet);
    }
    ...
    private void them() {
        if (check()) {
            ...
            int kt = new HoaDonChiTietRepo().them(new HoaDonChiTiet(mahd, maVe, soGheDat));
            if (kt == 1) {
                bang();
                HoaDonChiTiet hdct = new HoaDonChiTietRepo().timHDCTToDen(mahd, maVe);
    ...
    private void capNhat() {
        if (!txtMaHoaDon.getText().isEmpty() && !txtMaVeMB.getText().isEmpty()) {
            ...
            String maVe = txtMaVeMB.getText().trim();
            int soGheDat = Integer.parseInt(String.valueOf(txtSoGheDat.getText().trim()));
            int kt = new HoaDonChiTietRepo().sua(new HoaDonChiTiet(mahd, maVe, soGheDat));
            if (kt == 1) {
                bang();
    ...
    private void xoa() {
        if (dong >= 0) {
            ...
            if (r == JOptionPane.YES_OPTION) {
                String mahd = txtMaHoaDon.getText().trim();
                String maVe = txtMaVeMB.getText().trim();
                int kt = new HoaDonChiTietRepo().xoa(new HoaDonChiTiet(mahd, maVe));
                if (kt == 1) {
                    bang();
                } else {
    ...
    private void hienThi(int row) {
        ...
        new HoaDonChiTietRepo().hienThi(tblHoaDonChiTiet, hdct, row);
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        new HoaDonChiTietRepo().tim(tblHoaDonChiTiet, txtTimKiem.getText().trim());
        editColumnWidth();
